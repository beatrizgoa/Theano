...
def evaluate_lenet5_RGB(learning_rate, n_epochs, nkerns, batch_size, data, out_path):
    ...
    sal_capa7_valid = [salidas_capa7_valid(i) for i in range(n_valid_batches)]

    for i in sal_capa7:
        for j in i:
            input_test.append(j)
    for i in sal_capa7_valid:
        for j in i:
    ...
    print ('RGB model test losses:', test_score)
    ...
    print ('RGB best model obtained at iter:', best_iter,'whit a valid loss:', best_validation_loss)     

    return input_TrainClass, input_test, input_valid
