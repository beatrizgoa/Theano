...
def evaluate_lenet5(learning_rate=0.01, n_epochs=600, nkerns=[96, 256, 386, 384, 256], batch_size=20):
    ...
    print ('The configuration of the net is', 'learning_rate=', learning_rate, 'n_epochs=', n_epochs, 'nkerns=', nkerns, 'batch_size=', batch_size)
    ...
    layer0_input = x.reshape((batch_size, 4, 128, 128))
    ...
    layer0 = LeNetConvPoolLRNLayer(
        rng,
        input=layer0_input,
        image_shape=(batch_size, 4, 128, 128),
        filter_shape=(nkerns[0], 4, 11, 11),
        stride=(1, 1),
        lrn=True,
        poolsize=(2, 2),
    ...
    SVM_pred, SVM_pred_prob, scores_SVM = SVMClas(input_TrainClass, y_train, input_test, y_test, 'linear')
